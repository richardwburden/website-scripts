...
#Fetch the non-responsive archive issue index
if (not defined $options{'vi'} or ((length($options{'vi'} != 4) and (length($options{'vi'} != 6))
{
    usage;
}
if (length($options{'vi'} == 4)
{
    $zvol = substr($options{'vi'},0,2);
    $zissue = substr($options{'vi'},2);
    $year = $zvol + 1973; 
}
else
{
    $year =  substr($options{'vi'},0,4);
    $zvol = $year - 1973;
    $zissue = substr($options{'vi'},4);
}

my $ydir = $docroot.$year;
...
if ($is_directory)
{
    my $globpath = "$ydir/eirv$zvol"."n$zissue*";
    print "globpath: $globpath\n";
    @dirs = glob ($globpath);
}
else
{die "Directory $ydir not found."}

if (@dirs < 1)
{die "Directory matching $globpath not found."}

if (@dirs > 1)
{die "More than 1 directory matching $globpath found."}

my $nrIndexPath = $dir[0];
my $mech = WWW::Mechanize->new();
$mech->get($domainName.$nrIndexPath);
