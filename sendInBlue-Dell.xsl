...
<xsl:variable name="wspath" select="'http://localhost:8000/Dropbox_insecure/Dropbox/software/website-scripts/website-scripts/'" />

<xsl:variable name="unlisted" select="if ($hashdir eq '') then '' else 'unlisted/'" />

<xsl:variable name="sipath" select="concat('http://localhost:8000/Users/rburden/data/websites/EIR/eiw/public/',$unlisted,$yeardir,$issuedir,$hashdir,'index.xhtml')" />
<xsl:variable name="wwwsipath" select="concat('https://larouchepub.com/eiw/public/',$unlisted,$yeardir,$issuedir,$hashdir)" />

<xsl:variable name="rowuri" select="concat($wspath,'sendInBlueEmail-row-template-notext-noanchor-Dell.xhtml')" />
...
<xsl:variable name="srowuri" select="concat($wspath,'sendInBlueEmail-section-row-template-notext-noanchor-Dell.xhtml')" />
