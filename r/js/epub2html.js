...
function epub2html()
{
	...
	for (var k=0; k<sources.length; k++)
	    {var contents = sources.eq(k).contents();
		var newdiv = $("<div class='figure-source'></div>");
		newdiv.append(contents);
		sources.eq(k).replaceWith(newdiv);
		}
	...
	var overviews = layouts.eq(i).find(':regex(class,.*overview.*|figure-units)');
 	var sources = layouts.eq(i).find('.figure-source');
	...
	for(var j=0; j<imgs.length; j++)
		...
		if (!imgs.eq(j).parent().is('a'))
		    ...
		    if (imgs.length > 1)
			{
			    ...
			    for (var k=0; k<sources.length; k++)
				{
				    var simg = sources.eq(k).closestPrior('img');
				    console.log ("comparing with " + simg.outerHTML());
				    if (imgclone.attr('src') == simg.attr('src'))
					{
					    imgwrapper = checkTextWidth(sources.eq(k),maxTextWidth,imgwrapper,imgwrapper2);
