...
xmlns:fn="http://www.w3.org/2005/xpath-functions">
  ...
  <xsl:template match="node()|@*" priority="1" mode="tocArticle">
      <xsl:param name="textnode" tunnel="no" />
    <xsl:param name="dirkey" tunnel="no" />
    <xsl:param name="newtext" tunnel="no" />
  <xsl:copy>
      <xsl:apply-templates select="node()|@*"/>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="link" priority="2">
    <xsl:element name="link">
      ...
      <xsl:attribute name="href"><xsl:value-of select="concat('../../css/',substring-after(@href,'css/'))" /></xsl:attribute>
    </xsl:element>
  </xsl:template>
  ...
  <xsl:template match="p[@class='tocThumbnail']/a" priority="2">
    <xsl:copy>
      <xsl:attribute name="href"><xsl:value-of select="substring-after(@href,$issuedir)" /></xsl:attribute>
  ...
  <xsl:template match="script" priority="2">
    <xsl:element name="script">
      <xsl:attribute name="src"><xsl:value-of select="concat('../../css/',substring-after(@src,'css/'))" /></xsl:attribute>
    </xsl:element>
  </xsl:template>
  ...
  <xsl:template match="h3[@class='tocFullIssueView']" priority="2">
    <xsl:copy>
      <xsl:attribute name="class"><xsl:value-of select="'tocFullIssueViewSub'" /></xsl:attribute>
      <xsl:apply-templates select="node()|@*"/>
    </xsl:copy>
    <xsl:element name="h3">
      <xsl:attribute name="class"><xsl:value-of select="'tocSubscribersOnly'" /></xsl:attribute>
  ...
  <xsl:template match="div[@id='fullIssueLinks']/button" priority="2">
    <xsl:element name="button">
      ...
      <xsl:attribute name="class"><xsl:text>buttonA</xsl:text></xsl:attribute>
      <xsl:attribute name="onclick"><xsl:text>window.location='</xsl:text><xsl:value-of select="concat('../../../private/',$yeardir,$issuedir,substring-after(@onclick,$issuedir))" /></xsl:attribute>
...
use value of "id" if available; otherwise set it to "clear" -->
  ...
  <xsl:template match="h3[@class='tocArticle']" priority="2">
    ...
    <xsl:variable name="newdirkey" select="substring-before($textnode,'##')"/>
	 <xsl:variable name="trimdirkey" select="replace(replace($newdirkey,' +$',''),'^ +','')" />
    <xsl:variable name="linktext" select="if (contains($textnode,'##')) then substring-after($textnode,'##') else $textnode"/>
	 <xsl:variable name="trimlinktext" select="replace(replace($linktext,' +$',''),'^ +','')" />
	  <xsl:copy>
	<xsl:apply-templates select="node()|@*" mode="tocArticle">
      <xsl:with-param name="textnode" select="$textnode" tunnel="yes" />
      <xsl:with-param name="dirkey" select="$trimdirkey" tunnel="yes" />
      <xsl:with-param name="newtext" select="$trimlinktext" tunnel="yes" />
    </xsl:apply-templates>
		  <xsl:comment>dirkey: <xsl:value-of select="concat('%',$trimdirkey,'%')"></xsl:value-of></xsl:comment>
		  </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="a[@class='tocLinkPDF']" mode="tocArticle" priority="2">
    ...
    <xsl:copy>
      ...
      <xsl:choose>
        <xsl:when test="not (exists ($newdirs/dir[@key = $dirkey]))">
          <!-- subscriber only -->
      ...
      <xsl:apply-templates select="node()|@* except (@href,@class)" mode="newtext">
        <xsl:with-param name="textnode" select="$textnode" tunnel="yes" />
        <xsl:with-param name="newtext" select="$newtext" tunnel="yes" />
      </xsl:apply-templates>
  ...
  <xsl:template match="a[@class='tocLinkAltHTML']" mode="tocArticle" priority="2">
    ...
    <xsl:copy>
      <xsl:attribute name="href">
      <xsl:choose>
        <xsl:when test="exists ($newdirs/dir[@key = $dirkey])">
          <xsl:value-of select="concat($relativeRootDir,$newdirs/dir[@key = $dirkey]/@value,$yeardir,substring-before(@href,'.html'),$newFnSuffixes/sfx[@key = $dirkey]/@value,'.html')" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="'#'" />
        </xsl:otherwise>
      ...
      <xsl:apply-templates select="node()|@* except (@href,@class)"/>
		<xsl:comment>dirkey: <xsl:value-of select="concat('%',$dirkey,'%')"></xsl:value-of></xsl:comment>
    </xsl:copy>
  </xsl:template>
</xsl:stylesheet>
