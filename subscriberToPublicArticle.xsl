...
<!-- This stylesheet transforms an unlisted EIR article page for subscribers into a public EIR article page -->
...
<xsl:template match="link" priority="2">
	<xsl:element name="link">
		...
		<xsl:attribute name="href">
			<xsl:value-of select="base-uri(concat($wwwsipath,@href))" />
		</xsl:attribute>
	</xsl:element>
</xsl:template>
