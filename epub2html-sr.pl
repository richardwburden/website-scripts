...
specialReport|sr=s  (special folder name for special report, instead of the usual folder name computed from the volume number, issue number and date)
...
if (defined $specialReport) {$issue = '0'};
...
#Fetch the index with links to the PDFs
	...
	if ($pdfIndexPath =~ m%\]\]\]$%)
	{$pdfIndexPath =~ s%\]\]\]$%%}
	elsif (not defined $specialReport)
	{$pdfIndexPath .= "public/$year/$tenissueGroup/$yearIssue/"}
	else
	{
	    die "special reports require complete pdfIndexPath in config file at $configPath"
	}
	...
	my $arch_outfilepath = $piiarp;
	if (defined $specialReport)
	{
	    $year = $vol + 1973;
	    $arch_outfilepath .= "$year/$specialReport/index-nc.html";
	}
	else
	{
	    $arch_outfilepath .= "$year/eirv$zvol"."n$zissue-$year$zmonth$zmday/index-nc.html";
	}
...
#filename begins with a page number.
	...
	foreach $pdfLink (@pdfLinks)
	{
	    ...
	    if ($isPublicArchiveIndex and not defined $specialReport)
...
foreach $infilepath (@noncmInfiles)
{
	    ...
	    #use the default link to the issue index page in the EIR archive, which is under construction when the issue is new.
	{
	    ...
	    if (defined $specialReport)
	    {
		$issueIndexLink->attr('href',$piiarp."$year/$specialReport/index.html");
		$issueIndexLink->unshift_content("$specialReport, $year");
